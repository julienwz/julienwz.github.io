...
if(!class_exists('typography')) {
	class typography {
		...
		public function custom_styles($typography, $is_admin, $is_customize) {
			...
			// styles list
			$styles_list = array('color', 'text-transform', 'font-family', 'font-size', 'line-height', 'letter-spacing', 'text-decoration', 'text-decoration-color', 'background-color', 'text-stroke', 'padding', 'border-color', 'border-width', 'border-style');
			...
			if(isset($typography['custom']) && is_array($typography['custom'])) {
				...
				foreach ($typography['custom'] as $id => $style) {
					...
					// remove line height if not customize from inline
					if($element == 'span') {
						unset($styles_list[4]);
					}
					// start output
